...
class Equation {
    ...
    public function culc($a, $b, $c, $d){
        if (is_numeric($a) && is_numeric($b) && is_numeric($c) && is_numeric($d)){
        if ($a != 0) return $this->cube($a, $b, $c, $d);
        if ($b != 0) return $this->square($b, $c, $d);
        if ($c != 0) return $this->line($c, $d);
        if ($d != 0) return true;
        return false;
        } else {
